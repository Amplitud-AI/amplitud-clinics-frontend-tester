...
export type ClinicRagPanelProps = {
  bearer: string | null;
  effectiveClId: string;
  onLog: (title: string, body: string) => void;
};

export default function ClinicRagPanel({
  bearer,
  effectiveClId,
  onLog,
}: ClinicRagPanelProps) {
  ...
  /** Clinic knowledge (platform ``pt_id`` = default). */
  const queueTargetPtId = "";
  ...
  const wipeAllClinicRag = useCallback(async () => {
    if (!canClinicQueue) {
      onLog("Wipe all clinic RAG", "Need Bearer + anon + cl_id.");
      return;
    }
    if (wipeConfirmText !== "DELETE ALL") {
      onLog("Wipe all clinic RAG", "Confirmation text must be exactly DELETE ALL");
      return;
    }
    setBusy(true);
    try {
      const r = await clinicPostgrest("DELETE", `rest/v1/knowledge_documents_projection?cl_id=eq.${encodeURIComponent(cid)}&source_type=eq.clinic_document`, { bearer: token, anon });
      onLog("Wipe all clinic RAG", `projection delete status: ${r.status}`);
  ...
  }, [anon, canClinicQueue, cid, onLog, refreshKnowledgeProjection, token, wipeConfirmText]);
  ...
  return (
    <section className="border border-zinc-300 dark:border-zinc-600 rounded p-4 space-y-4">
      <h2 className="font-medium">Phase F — Clinic Knowledge / RAG</h2>
        ...
        <div className="rounded border border-zinc-200 dark:border-zinc-700 bg-white/70 dark:bg-zinc-950/40 p-3 space-y-2">
          ...
          {showWipeModal && (
            <div className="p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 rounded space-y-2 mb-2">
              <p className="text-xs text-red-900 dark:text-red-200 font-medium">Wipe all clinic-wide RAG data?</p>
              <p className="text-[11px] text-red-700 dark:text-red-300">
                Type <strong>DELETE ALL</strong> to confirm. This will leave patient rows intact.
              </p>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={wipeConfirmText !== "DELETE ALL" || busy}
                  onClick={() => void wipeAllClinicRag()}
                >
                  Confirm Wipe
                </button>
                <button
