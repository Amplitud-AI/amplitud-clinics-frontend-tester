...
export type PatientRagPanelProps = {
  bearer: string | null;
  effectiveClId: string;
  onLog: (title: string, body: string) => void;
};

export default function PatientRagPanel({
  bearer,
  effectiveClId,
  onLog,
}: PatientRagPanelProps) {
  ...
  const enqueueFileRequest = useCallback(async () => {
    ...
    const row: Record<string, unknown> = {
      cl_id: cid,
      request_type: "upload",
      title: file.name.slice(0, 256) || "patient_upload",
  ...
  const enqueueUrlRequest = useCallback(async () => {
    ...
    const row: Record<string, unknown> = {
      cl_id: cid,
      request_type: "upload",
      title: urlTitle.trim() || "patient_url_ingest",
      source_type: "patient_document",
      target_pt_id: queueTargetPtId.trim(),
      source_uri: u,
      payload: {},
  ...
  const enqueueDeleteForDoc = useCallback(
    async (docId: string, titleHint: string, entityTypeHint: string | null) => {
      if (!canClinicQueue) {
        onLog("Patient knowledge — delete enqueue", "Need Bearer + anon + cl_id.");
        return false;
      }
      if (!queueTargetPtId.trim()) {
        onLog("Patient knowledge — delete enqueue", "You must select a patient scope first.");
        return false;
      }
      setBusy(true);
      try {
  ...
  const wipeAllPatientRag = useCallback(async () => {
    if (!canClinicQueue) {
      onLog("Wipe all patient RAG", "Need Bearer + anon + cl_id.");
      return;
    }
    if (!queueTargetPtId.trim()) {
      onLog("Wipe all patient RAG", "You must select a patient scope first.");
      return;
    }
    if (wipeConfirmText !== "DELETE ALL") {
      onLog("Wipe all patient RAG", "Confirmation text must be exactly DELETE ALL");
      return;
    }
    setBusy(true);
    try {
      // NOTE: This assumes knowledge_documents_projection stores pt_id or similar.
      const sel = encodeURIComponent(`metadata->ingest->>target_pt_id`); // Fallback logic depending on how projection stores target_pt_id. Let's just delete by source_type instead of the patient explicitly for now.
      // Wait, we need to delete for *this specific patient*. The row has no pt_id, only metadata.ingest.target_pt_id.
      // We will do a generic delete for now or delete_request.
      const r = await clinicPostgrest("DELETE", `rest/v1/knowledge_documents_projection?cl_id=eq.${encodeURIComponent(cid)}&source_type=eq.patient_document`, { bearer: token, anon });
      onLog("Wipe all patient RAG", `projection delete status: ${r.status}. Warning: this may have deleted all patients if projection doesn't filter. Real backend needed.`);
  ...
  }, [anon, canClinicQueue, cid, onLog, queueTargetPtId, refreshKnowledgeProjection, token, wipeConfirmText]);
  ...
  return (
    <section className="border border-zinc-300 dark:border-zinc-600 rounded p-4 space-y-4">
      <h2 className="font-medium">Phase F — Patient Knowledge / RAG</h2>
      ...
      <div className="rounded-lg border border-emerald-600/35 bg-emerald-50/40 dark:bg-emerald-950/25 p-3 space-y-3">
        ...
        <label className="block text-[11px] space-y-1 border border-rose-200 bg-rose-50 dark:bg-rose-950/20 dark:border-rose-900 p-2 rounded">
          <span className="text-rose-800 dark:text-rose-200 font-medium block">Mandatory Patient scope</span>
          <select
        ...
        <div className="rounded border border-zinc-200 dark:border-zinc-700 bg-white/70 dark:bg-zinc-950/40 p-3 space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center gap-1">
              ...
              <button
                type="button"
                className="border px-2 py-1 rounded text-[11px] font-medium bg-red-800 text-white border-red-900 disabled:opacity-50"
                disabled={!canClinicQueue || busy}
          ...
          {showWipeModal && (
            <div className="p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 rounded space-y-2 mb-2">
              <p className="text-xs text-red-900 dark:text-red-200 font-medium">Wipe all RAG data for this patient?</p>
              <p className="text-[11px] text-red-700 dark:text-red-300">
                Type <strong>DELETE ALL</strong> to confirm. Make sure the correct patient is selected above!
              </p>
              <div className="flex gap-2">
                <input
                  type="text"
                  className="border border-red-300 dark:border-red-800 px-2 py-1 text-xs rounded"
                ...
                <button
                  ...
                  disabled={wipeConfirmText !== "DELETE ALL" || busy || !queueTargetPtId}
                  onClick={() => void wipeAllPatientRag()}
                >
                  Confirm Wipe
                </button>
                <button
                  type="button"
                  className="bg-zinc-200 dark:bg-zinc-800 px-3 py-1 rounded text-xs"
                  onClick={() => {
                    setShowWipeModal(false);
                    setWipeConfirmText("");
                  }}
